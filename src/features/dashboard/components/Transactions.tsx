        ...
        {items.map((transaction) => (
          ...
          >
            <div className="flex min-w-0 items-center gap-3">
              <div className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-full ${accentMap[transaction.accent]}`}>
                <i className={transaction.icon} />
              </div>
              <div className="min-w-0">
                <p className="truncate text-sm font-semibold text-slate-900 dark:text-white">{transaction.title}</p>
                <p className="text-xs text-slate-500 dark:text-slate-400">{transaction.time}</p>
              </div>
            </div>
            <span className={`ml-4 shrink-0 text-sm font-semibold ${transaction.type === 'income' ? 'text-brand-secondary' : 'text-red-500 dark:text-red-400'}`}>
              {transaction.amount}
            </span>
          </button>
        ))}
